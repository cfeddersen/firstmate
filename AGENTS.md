...
Use its scaffold as the contract, then fill every placeholder before dispatch or seeding: `{TASK}` takes the task description, acceptance criteria, constraints, and necessary context, and `{OUT_OF_SCOPE}` takes the work the task must refuse.
