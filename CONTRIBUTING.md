...
Firstmate's wrapper still matters: crewmates route `ask-user` findings to firstmate except for the narrow worker self-resolution carve-out inside `ask-user-authority`, and crewmates avoid `--yes` because it would bypass that check and any required captain escalation.
