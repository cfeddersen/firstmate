...
| `fm-dispatch-resolve.sh` | Resolve one concrete crewmate or scout dispatch profile from a written brief with typesafe.ai's System One model (Jev), opt-in through `TYPESAFE_API_KEY` (docs/configuration.md "Typed dispatch resolution") |
