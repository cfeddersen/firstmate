...
With `state/.afk` absent or away mode off the daemon lock proves nothing and the strict watcher predicate is unchanged.
Away-mode coverage at the guard is never the flag alone: `block_stop` renders the mode-aware repair line (quiet-mode wording for a `quiet` flag) and the `--claude` attended fail-open stays excluded only when `state/.afk` is present and `fm_afk_supervision_covered` in `bin/fm-afk-start.sh` confirms the away-mode daemon lock is held by a live daemon.
