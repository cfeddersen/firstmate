...
test_hook_away_daemon_allows_beacon_within_poll_derived_grace() {
  ...
  beat=$(( $(date +%s) - 400 ))
  set_poll_grace_beacon_mtime "$dir/state/.last-watcher-beat" "$beat"
...
test_hook_away_daemon_blocks_dead_daemon_despite_poll_derived_grace() {
  ...
  # A dead daemon cannot own supervision; the retained flag needs ordinary watcher repair.
  assert_contains "$out" "$REQUIRED_REASON" "a dead daemon must point at ordinary watcher repair"
